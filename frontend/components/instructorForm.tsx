...
const InstructorForm = ({ method, instruments, courses, values } : InstructorFormProps) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("/api/submit?entity=instructors", request);
    ...
    if (result.error) {
      ...
      alertService.error(`Could not ${action} instructor (error ${error}: ${errorMessage})`, { id: "main" })
    }
    else {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <fieldset>
        ...
        <input type="text" name="first_name" id="first_name" className="mb-3" /><br />
        <label htmlFor="last_name">Last Name</label><br />
        <input type="text" name="last_name" id="last_name" className="mb-3" /><br />
      </fieldset>
          ...
          <fieldset>
            ...
            <div className="max-h-36 max-w-fit overflow-y-scroll">
              {Object.entries(instruments).map(
                (entry => (
                  <><input type="checkbox" name="instrument" id="instrument" key={entry[1]} value={entry[1]} />
                  <label key={entry[0]} htmlFor={entry[1]} className="px-2">{entry[1]}</label><br /></>
                ))
              )}
            </div>
          </fieldset>
          ...
          <fieldset>
            ...
            <div className="max-h-36 max-w-fit overflow-y-scroll">
              {Object.entries(courses).map(
                (instrument => (
                  <><div key={instrument[0]} className="pb-1">{instrument[0]}</div>
                  {Object.entries(instrument[1]).map(
                    (entry => (
                      <><input type="checkbox" name="course" id="course" key={entry[1]} value={entry[1]} />
                      <label key={entry[0]} htmlFor={entry[1]} className="px-2">{entry[1]}</label><br /></>
                    ))
                  )}
                  </>
